...
- 優先度キュー操作 (`heapq.heappop/push`) を Rust 側 (例: `std::collections::BinaryHeap`) で完結させ、ヒープ要素構造体とmm、」構造体と PyO3 連携を定義する。ホットスポット上位にあり Rust 化候補として指名済み。[specs/001-plan/runtime-profiling-report.md#L29-L39](specs/001-plan/runtime-profiling-report.md#L29-L39)
...
- Rust化後のプログラムをviztracerで計測し、Rust化したことで削減された処理時間を評価しレポートに記す
